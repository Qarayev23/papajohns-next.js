...
export const basket = createSlice({
  ...
  initialState,
  reducers: {
    removeFromBasket: (state, action: PayloadAction<{ id: number; price: number; count: number; }>) => {
    ...
    increment: (state, action: PayloadAction<{ id: number; price: number; }>) => {
      state.basketItems = state.basketItems.map(item => item.id === action.payload.id ?
    ...
    addBaket: (state, action: PayloadAction<BasketProps>) => {
      const check = state.basketItems.some(item => item.id === action.payload.id)
      if (!check) {
